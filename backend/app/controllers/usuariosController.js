...
async function store(req, res) {
    ...
    try {
        ...
        // res.json({datos});
        const [respuesta] = await c.query('INSERT INTO usuarios (nombre_usuario, contraseña, rol, estado) VALUES (?,?,?,?) ',
            [datos.nombre_usuario, datos.contraseña, datos.rol, datos.estado]);
...
async function update(req, res) {
    ...
    try {
        ...
        var id = req.params.id;
        const datos = req.body;
        const [respuesta] = await c.query('UPDATE usuarios SET nombre_usuario=?, contraseña=?, rol=?, estado=? WHERE id_usuario=?',
            [datos.nombre_usuario, datos.contraseña, datos.rol, datos.estado, id]);
...
async function destroy (req, res) {
    ...
    try {
        ...
        const [respuesta] = await c.query('UPDATE usuarios SET estado=? WHERE id_usuario=?',
            ['Inactivo', id]);
        res.status(200).json({mensaje : 'Usuario dada de baja', datos : respuesta});
    } catch (error) {
        res.status(400).json({mensaje : 'Error en la consulta', error : error.message});
    }
};
